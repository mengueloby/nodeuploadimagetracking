...
usermethods.register = function(data,callback){
  ...
  // var pp = typeof(data.payload.pp) == 'string' && data.payload.pp.trim().length > 0 ? data.payload.pp.trim() : false;
  console.log(data.payload.pp);
    
    fs.writeFile('test.jpg', data.payload.pp, function (err) {
      if (err) throw err;
      console.log(data.payload.pp.name+' is either created or if exists then updated');
    });
